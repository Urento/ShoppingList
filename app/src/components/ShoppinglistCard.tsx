...
export const ShoppinglistCard: React.FC = ({}) => {
  ...
  const updateTitle = async (id: number, owner: string) => {
    await swal({
      ...
      {shoppinglists.map((e: Shoppinglist) => (
        <div className="pt-2 pl-2">
          <div className="max-w-md py-4 px-8 bg-gray-800 shadow-lg rounded-lg">
            ...
            <div className="justify-center md:justify-end -m-9 pl-96">
              <button onClick={() => updateTitle(e.id, e.owner)}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-7 object-cover rounded-full text-red-600"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
