...
func TestCheckAuth(t *testing.T) {
	...
	t.Run("When Account is Disabled", func(t *testing.T) {
		err = auth.DisableAccount()
		if err != nil {
			t.Errorf("Error while disabling account: %s", err)
		}

		disabledBefore, err := auth.IsDisabled()
		if err != nil {
		...
		disabledAfter, err := auth.IsDisabled()
		if err != nil {
			t.Errorf("Error while checking if the account is disabled: %s", err)
		}

		err = auth.ActivateAccount()
		if err != nil {
	...
	t.Run("Wrong Password", func(t *testing.T) {
		wrongCredentialsAuth := Auth{
			EMail:    email,
			Password: "khdfgbskdjgbdfsgjkdsbgfj",
			Username: "dngkjnsdgf",
		}
		ok, _ := wrongCredentialsAuth.Check()
...
func TestUpdateUsername(t *testing.T) {
	...
	t.Run("Update Username", func(t *testing.T) {
		...
		username := util.StringWithCharset(30)
	...
	t.Run("Update Username with over 32 characters", func(t *testing.T) {
		...
		username := util.StringWithCharset(34)
		updateUsernameAuthObj := Auth{
			EMail:    email,
			Username: username,
		}
